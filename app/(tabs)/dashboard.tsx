...
interface Route {
  ...
  destination?: string;
  safetyRating: number;
  [key: string]: any; // For additional properties from Firestore
}
...
const DashboardScreen: React.FC = () => {
  ...
  useEffect(() => {
    getUserLocation();
    const routesUnsubscribe = subscribeToRoutes();
    const updatesUnsubscribe = subscribeToUpdates();
  ...
  const subscribeToRoutes = (): Unsubscribe => {
  ...
  const subscribeToUpdates = (): Unsubscribe => {
    ...
    const unsubscribe = onSnapshot(
      updatesQuery,
      (snapshot) => {
        const updatesData: Update[] = snapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        })) as Update[];
  ...
  const handleSafetyCheck = async (isSafe: boolean): Promise<void> => {
    if (!userLocation) {
    ...
    try {
      ...
      Alert.alert(
        ...
        isSafe
          ? "Glad you're safe!"
          : "Stay alert! Help is available if needed.",
        [{ text: "OK" }]
      );
    } catch (error) {
      ...
      >
        ...
        <View style={styles.section}>
          ...
          {updates.length > 0 ? (
            ...
            <View style={styles.emptyState}>
              <Ionicons name="newspaper-outline" size={48} color="#bdc3c7" />
